...
public class Oauth2ServerInterceptor extends InterceptorAdapter {
    ...
    public boolean incomingRequestPostProcessed(RequestDetails theRequestDetails, HttpServletRequest theRequest, HttpServletResponse theResponse) throws AuthenticationException {
        ...
        //We get the list of scopes from the scope claim from the AT
        ...
        for(String scopeFromToken: scopesFromToken) {
            if(hasValidFormat(scopeFromToken)) {
                ...
                String op = scopeFromToken.substring(id.length()+1+scopeResource.length()+1, scopeFromToken.length());
                if(scopeFromToken != null) {
                    //Checking for the equivalence between HTTP GET, PUT, POST and FHIR Scopes read, write & *
                    if(checkOp(theRequestDetails.getRequestType(), op)) {
                        ...
                        if(checkResource(theRequestDetails.getResourceName(), scopeResource)) {
                            //Checking the level of the scope, between general user scope, or patient limited
                            if (id.equals("user")) {
                                return true;
    ...
    private boolean verifySignature(SignedJWT idToken, JWTClaimsSet idClaims) {
        try {
            ...
            if (issuerUrl == null) {
                ...
                throw new AuthenticationException("Not authorized (no server configuration found for issuer " + issuerUrl + ")");
            }
            ...
            if (issuerAsJsonObject == null) {
                ...
                throw new AuthenticationException("Not authorized (no issuer json response found for issuerUrl " + issuerUrl + ")");
            }
            ...
            if(alg.equals(JWSAlgorithm.HS256) || alg.equals(JWSAlgorithm.HS384) || alg.equals(JWSAlgorithm.HS512)) {
                ...
                if(publicKeyAsString == null || publicKeyAsString.equals("")) {
                    ...
                    throw new AuthenticationException("Not authorized (no public key found for issuer object " + issuerAsStringObject + ")");
                }
                ...
                return verifier.verify(new JWSHeader(idToken.getHeader().getAlgorithm()), idToken.getSigningInput() , idToken.getSignature());
            } else {
    ...
    public boolean checkOp(RequestTypeEnum requestTypeEnum, String scopeOp) {
        ...
        case GET:
            if (scopeOp.equals("read")
                || scopeOp.equals("*")) {
                return true;
            }
            break;
        case POST:
            if (scopeOp.equals("write")
                || scopeOp.equals("*")) {
                return true;
            }
            break;
